import time # For adding delays or wait periods during automation
from selenium import webdriver  # Provides the main WebDriver interface to control browser
from selenium.webdriver.chrome.service import Service # Manages the service for launching Chrome
from selenium.webdriver.common.by import By  # Allows selecting elements using locators like ID, NAME, XPATH
from selenium.webdriver.common.keys import Keys  # Lets you send special keys like ENTER, ESCAPE, etc.
...
def search_google(query):
    ...
    try:
        ...
        # Start timer for waiting & CAPTCHA handling    
        max_wait = 180
        check_interval = 5
        elapsed = 0
        snippet = None
        ...
        # Wait loop to allow time for Google results or CAPTCHA resolution
        while elapsed < max_wait:
            ...
            # Look for snippet in known selectors
            for sel in selectors:
                elements = soup.select(sel)
                for element in elements:
                    text = element.get_text(strip=True)
                    if text and len(text.split()) > 3:  # Ensure it's not just one-word
                        snippet = text
                        print(f"Snippet found: {snippet}")
                        break
                if snippet:
                    break
            # Fallback if no snippet found—use top result title          
            if not snippet:
